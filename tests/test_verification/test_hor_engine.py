...
class TestHORRuleEngine:
    ...
    @pytest.mark.asyncio
    async def test_run_all_returns_64_results(self) -> None:
        engine = HORRuleEngine()
        results = await engine.run_all({"title": "Test PRD", "content": "", "sections": []})
        assert len(results) == TOTAL_RULES

    @pytest.mark.asyncio
    async def test_run_by_category(self) -> None:
        engine = HORRuleEngine()
        structural = await engine.run_by_category("structural", {"title": "Test", "content": ""})
        assert len(structural) == 10

    @pytest.mark.asyncio
    async def test_run_single(self) -> None:
        engine = HORRuleEngine()
        result = await engine.run_single(1, {"title": "Valid Title"})
        assert result.rule_id == 1
        assert result.passed is True

    @pytest.mark.asyncio
    async def test_penalty_calculation_no_failures(self) -> None:
        engine = HORRuleEngine()
        results = await engine.run_all({"title": "Test", "content": ""})
        passing = [r for r in results if r.passed]
        score = engine.calculate_adjusted_score(1.0, passing)
        assert score == 1.0

    @pytest.mark.asyncio
    async def test_penalty_calculation_with_failures(self) -> None:
        engine = HORRuleEngine()
        results = await engine.run_all({"content": ""})
