...
class TestVerificationPipeline:
    ...
    @pytest.mark.asyncio
    async def test_empty_artifact_fails_critical_rules(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_adjusted_score_decreases_on_failures(self) -> None:
        """Score should decrease when rules fail."""
        engine = HORRuleEngine()

        good_results = await engine.run_all({
            "title": "Good PRD",
        ...
        bad_results = await engine.run_all({})
    ...
    @pytest.mark.asyncio
    async def test_all_rule_ids_sequential(self) -> None:
        """Verify rules 1-64 are all registered with no gaps."""
        engine = HORRuleEngine()
        results = await engine.run_all({"title": "Test"})
